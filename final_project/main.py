...
def train(env):
    ...
    ppo_agent = PPO(state_dim, action_dim,h,l)
    ...
    torch.save(ppo_agent.policy.state_dict(), 'ppo_robotarm_model_r.pth')#, '../test_yt/ppo_robotarm_model.pth')
    print("Save")
    # Plotting loss and reward per episode
    plt.figure(figsize=(12, 6))
    plt.subplot(2, 1, 1)
    plt.plot(episode_losses)
    plt.title('Episode_r Loss per Episode')
    plt.xlabel('Episode')
    plt.ylabel('Loss')
