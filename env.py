...
Y_range = [-500, 500]
Z_range = [50, 650]
Points = get_point() #get target_point , obstacle point
...
class RobotArmEnv(gym.Env):
    ...
    #         reward += 1000
    #         done = True
    #     if any(distance < Safe_distance for distance in distances): #collision
    #         #print("hit !!")
    #         reward -= 5
    #     # Rb : to keep safe distance
    #     if any(distance > Safe_distance for distance in distances):
    #         d0 = min(distances) # d0 : the minimize distance of robot arm to obstacle
    #         rb = -beta * max(Safe_distance-d0 , 0 )
    #         reward += rb
    #
    #     return reward, done
    def calculate_reward_and_done(self, edge):
